...
# line 4 "[EC]/@PLUGIN_KEY@-@PLUGIN_VERSION@/startAppPool.pl"
# -------------------------------------------------------------------------
# File
# startAppPool.pl
#
...
# is passed to cscript).
    ...
    try {
        appPool.Start();
        WScript.Echo("Application Pool $appPoolName started successfully");
    }
    catch(e) {
...
EOSCRIPT
    ...
    if ( $? == SUCCESS ) {
        ...
        if ( $content =~ m/Application Pool (.+) started successfully/ ) {
