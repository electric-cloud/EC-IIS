...
# line 4 "[EC]/@PLUGIN_KEY@-@PLUGIN_VERSION@/createAppPool.pl"
# -------------------------------------------------------------------------
# File
# createAppPool.pl
#
...
# is passed to cscript).
    ...
    // Create the app pool
    try {
        var appPool = appPools.Create("IIsApplicationPool", "$appPoolName");
        appPool.SetInfo();
        WScript.Echo("Application Pool $appPoolName created successfully");
    }
    catch(e) {
