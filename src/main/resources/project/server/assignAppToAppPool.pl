...
# line 4 "[EC]/@PLUGIN_KEY@-@PLUGIN_VERSION@/assignAppToAppPool.pl"
#
...
#$cmdLine = "$appcmdLocation set app /app.name:\"$::gAppName\" /applicationPool:\"$::gAppPoolName\"";
    $cmdLine =
"$appcmdLocation set site /site.name:\"$::gSiteName\" /[path='$::gAppName'].applicationPool:\"$::gAppPoolName\"";
    ...
    if ( $? == SUCCESS ) {
        ...
        if ( $content =~ m/SITE object "(.+)" changed/ ) {
            print
"Application $::gAppName moved to Application Pool $::gAppPoolName\n";
        }
        else {
